...
void Model::set_ifelm (int g, int i, int m)
{
	...
	// MPER
	if (_qmon->write_avail () >= 3)
	{
	    _qmon->write (0, 0xB0); // Channel?
	    _qmon->write (1, MIDICTL_IFELM);
	    _qmon->write (2, 0x40 + ((s + 1) << 4) + (g & 7));
	    _qmon->write_commit (3);
            fprintf(stderr, "Wrote state %u, mode %u, group %u to monitor queue.\n", s, s + 1, g & 7);
	...
	if (_qmon->write_avail () >= 3)
	{
	    ...
	    _qmon->write (2, i & 31);
	    _qmon->write_commit (3);
            fprintf(stderr, "Wrote index %u to monitor queue.\n", i & 31);
	}
