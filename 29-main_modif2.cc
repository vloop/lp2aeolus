...
static Lfq_u8   fb_queue (1024); //MPER
static Iface   *iface;
...
#endif
    // MPER added &fb_queue to model
    model = new Model (&comm_queue, &midi_queue, &fb_queue, audio->midimap (), audio->appname (), S_val, I_val, W_val, u_opt);
    imidi = new Imidi (&note_queue, &midi_queue, audio->midimap (), audio->appname ());
    omidi = new Omidi (&fb_queue, audio->appname ()); // MPER
    slave = new Slave ();
...
//    ITC_ctrl::connect (model, EV_OMIDI, omidi, EV_OMIDI); // MPER
    ITC_ctrl::connect (model, TO_OMIDI, omidi, FM_MODEL); // MPER
    ...
    omidi->thr_start (SCHED_OTHER, 0, 0x00010000); // MPER
