...
Imidi::Imidi (Lfq_u32 *qnote, Lfq_u8 *qmidi, Lfq_u8 *qmon, uint16_t *midimap, const char *appname) :
    A_thread ("Imidi"),
    _qnote (qnote),
    _qmidi (qmidi),
    _qmon (qmon),
...
void Imidi::open_midi (void)
{
    ...
    if (snd_seq_open (&_handle, "hw", SND_SEQ_OPEN_DUPLEX, SND_SEQ_NONBLOCK) < 0) // MPER
    {
    ...
    fprintf(stderr, "Created sequencer input port.\n");

    if ((_opport = snd_seq_create_simple_port (_handle, "Out",
         // SND_SEQ_PORT_CAP_WRITE,
         SND_SEQ_PORT_CAP_READ|SND_SEQ_PORT_CAP_SUBS_READ, // MPER
         SND_SEQ_PORT_TYPE_APPLICATION)) < 0)
    {
        fprintf(stderr, "Error creating sequencer output port.\n");
        exit(1);
    }
    fprintf(stderr, "Created sequencer output port.\n");
...
void Imidi::proc_midi (void) 
{
    ...
    while (true)
    {
        ...
        while (_qmon->read_avail () >= 3)
	{
	    t = _qmon->read (0);
	    p = _qmon->read (1);
	    v = _qmon->read (2);
	    _qmon->read_commit (3);
            fprintf(stderr, "Received from monitor queue %X %X %X\n", t, p, v);
	    if ((t & 0xF0) == 0xB0)
	    {
		snd_seq_ev_clear(&ev);
	...
	e = snd_seq_event_input(_handle, &E);
        // fprintf(stderr, "%i", e);
	if (e != -EAGAIN) // (snd_seq_event_input_pending(_handle, 0))
	{
	    // snd_seq_event_input(_handle, &E); // Blocking??
	    ...
	    t = E->type;
            fprintf(stderr, "Received from alsa midi type %X\n", t);
	    switch (t)
	    { 
