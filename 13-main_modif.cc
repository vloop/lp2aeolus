...
static Lfq_u8   mon_queue (1024); // MPER
static Iface   *iface;
...
#endif
    model = new Model (&comm_queue, &midi_queue, &mon_queue, audio->midimap (), audio->appname (), S_val, I_val, W_val, u_opt);
    imidi = new Imidi (&note_queue, &midi_queue, &mon_queue, audio->midimap (), audio->appname ());
    slave = new Slave ();
