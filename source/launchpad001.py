...
class MidiInputHandler(object):
    ...
    def __call__(self, event, data=None):
        ...
        if message[0] == CONTROL_CHANGE + int(self.midi_channel_in):
            ...
            color = 0x2D if (message[2] == 0x7F) else 0
            print("Envoi:", NOTE_ON + self.midi_channel_out, note, color)
            self.out_port.send_message([NOTE_ON + int(self.midi_channel_out), note, color])
        else:
